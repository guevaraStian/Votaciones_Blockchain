...
def Candidato_Registro_2():
    if request.method == 'POST':
        ...
        judicial = request.form['judicial']
        ...
        if(colvacia == None):
            ...
            HashActual = ph.hash(NumeroBloque.encode()+cedula_candidato.encode()+nombre_candidato.encode()
                                   +partido_politico.encode()+fecha_nacimiento.encode()+direccion.encode()
                                   +telefono.encode()+foto.encode()+judicial.encode()+firma.encode()+HashAnterior.encode())
            ...
            col.insert_one({'NumeroBloque': NumeroBloque, 'cedula_candidato': cedula_candidato, 'nombre_candidato': nombre_candidato,
                        'partido_politico': partido_politico, 'fecha_nacimiento': fecha_nacimiento,'direccion': direccion,
                        'telefono': telefono,'foto': foto, 'judicial': judicial, 'firma': firma, 'HashAnterior': HashAnterior, 'HashActual': HashActual})
        ...
        return render_template('Candidato_Registro.html',NumeroBloque= NumeroBloque, cedula_candidato=cedula_candidato ,nombre_candidato=nombre_candidato , partido_politico= partido_politico ,
                               fecha_nacimiento= fecha_nacimiento , direccion= direccion,telefono= telefono,foto= foto,
                               judicial= judicial,firma= firma, HashAnterior=HashAnterior , HashActual=HashActual )
...
def Planilla_Mesa_Registro_2():
    if request.method == 'POST':
        ...
        nombre_opcion_1 = request.form['nombre_opcion_1']
        votacion_opcion_1 = request.form['votacion_opcion_1']
        nombre_opcion_2 = request.form['nombre_opcion_2']
        votacion_opcion_2 = request.form['votacion_opcion_2']
        votos_blanco = request.form['votos_blanco']
        total_votantes_habilitados = request.form['total_votantes_habilitados']
        votos_realizados = request.form['votos_realizados']
        firma_jurados = request.form['firma_jurados']
        ...
        if(colvacia == None):
            ...
            HashActual = ph.hash(NumeroBloque.encode()+departamento.encode()+numero_mesa.encode()+nombre_opcion_1.encode()+votacion_opcion_1.encode()
                                   +nombre_opcion_2.encode()+votacion_opcion_2.encode()+votos_blanco.encode()+total_votantes_habilitados.encode()
                                   +votos_realizados.encode()+firma_jurados.encode()+HashAnterior.encode())
            #Se insertan los valores en la base de datos
            col.insert_one({'NumeroBloque': NumeroBloque, 'departamento': departamento,'numero_mesa': numero_mesa,
                            'nombre_opcion_1': nombre_opcion_1, 'votacion_opcion_1': votacion_opcion_1,
                            'nombre_opcion_2': nombre_opcion_2, 'votacion_opcion_2': votacion_opcion_2,'votos_blanco': votos_blanco,
                            'total_votantes_habilitados': total_votantes_habilitados,'votos_realizados': votos_realizados,
                            'firma_jurados': firma_jurados,'HashAnterior': HashAnterior, 'HashActual': HashActual })
        if (colvacia != None):
            bloquecodificado = col.find().sort('NumeroBloque', -1).limit(1)
            #se recorren los items dentro de ese ultimo bloque 
            #cada item se guarda en la variable "bloqueanterior"
        ...
        return render_template('Planilla_Mesa_Registro.html', NumeroBloque=NumeroBloque ,departamento=departamento ,numero_mesa=numero_mesa ,
                               nombre_opcion_1=nombre_opcion_1 , votacion_opcion_1=votacion_opcion_1 ,
                               nombre_opcion_2= nombre_opcion_2 , votacion_opcion_2= votacion_opcion_2 , votos_blanco= votos_blanco, 
                               total_votantes_habilitados= total_votantes_habilitados , votos_realizados= votos_realizados , 
                               firma_jurados= firma_jurados , HashAnterior=HashAnterior , HashActual=HashActual)
...
def Voto_Individual_Registro_2():
    if request.method == 'POST':
        ...
        Sistema_Operativo_Votante = request.headers.get('User-Agent')
        Hora_Votacion = datetime.now().isoformat()
        #Conexion a la base de datos "Blockchain" y seleccion de la coleccion "Bloques"
        client = MongoClient('localhost', port=27017, username='', password='')
        db = client['VBlockchain']
        ...
        if(colvacia == None):
            ...
            HashActual = ph.hash(NumeroBloque.encode()+numero_cedula.encode()+nombre_votante.encode()
                                   +voto_pais.encode()+voto_departamento.encode()+voto_municipio.encode()+Ip_Votante.encode()
                                   +Sistema_Operativo_Votante.encode()+Hora_Votacion.encode()+HashAnterior.encode())
            #Se insertan los valores en la base de datos
            col.insert_one({'NumeroBloque': NumeroBloque, 'numero_cedula': numero_cedula, 'nombre_votante': nombre_votante,
                        'voto_pais': voto_pais, 'voto_departamento': voto_departamento, 'voto_municipio': voto_municipio,
                        'Ip_Votante': Ip_Votante,'Sistema_Operativo_Votante': Sistema_Operativo_Votante,'Hora_Votacion': Hora_Votacion, 
                        'HashAnterior': HashAnterior, 'HashActual': HashActual })
        ...
        if (colvacia != None):
            ...
            col.insert_one({'NumeroBloque': NumeroBloque, 'numero_cedula': numero_cedula, 'nombre_votante': nombre_votante,
                        ...
                        'HashAnterior': HashAnterior, 'HashActual': HashActual })
        #En formulario se retorna e imprime en pantalla la siguiente informacion
        return render_template('Voto_Individual_Registro.html',NumeroBloque= NumeroBloque, numero_cedula=numero_cedula,
                               nombre_votante=nombre_votante, voto_pais= voto_pais, voto_departamento= voto_departamento , 
                               voto_municipio= voto_municipio, Ip_Votante= Ip_Votante,Sistema_Operativo_Votante= Sistema_Operativo_Votante,
                               Hora_Votacion= Hora_Votacion,HashAnterior=HashAnterior, HashActual=HashActual )
...
def Graficas_Dinamica():
    ...
    col = db['Bloques']
    #Se consulta los bloques creados y se guarda en la variable "listabloques"
    colvacia2 = col.find_one({"NumeroBloque": "10"})
    # Dataset de ejemplo
    df = pd.DataFrame([colvacia2]) 
    # Convertir votaciones a números
    df['votacion_opcion_1'] = pd.to_numeric(df['votacion_opcion_1'])
    df['votacion_opcion_2'] = pd.to_numeric(df['votacion_opcion_2'])
    # Reorganizar datos para gráfico
    df_votos = pd.DataFrame({
        'Nombre': [df['nombre_opcion_1'][0], df['nombre_opcion_2'][0]],
        'Votos': [df['votacion_opcion_1'][0], df['votacion_opcion_2'][0]]
        })
    # Crear gráfico
...
def Confirmar_Votos_2():
    if request.method == 'POST':
        ...
        votacion_1_string = documento_total["votacion_opcion_1"] if documento_total else 0
        votacion_2_string = documento_total["votacion_opcion_2"] if documento_total else 0
